...
circle1 = (((LineZ-centerz).^2+(Linex).^2 >= (r1 - D/2)^2)...
    &((LineZ-centerz).^2+(Linex).^2 <= (r1 + D/2)^2));
parabol1 = (1 -abs(sqrt((LineZ-centerz).^2+(Linex).^2) - r1 ).^2 /(D/2)^2);

circle2 = (((LineZ-centerz).^2+(Linex).^2 >= (r2 - D/2)^2)...
    &((LineZ-centerz).^2+(Linex).^2 <= (r2 + D/2)^2));
parabol2 = (1 -abs(sqrt((LineZ-centerz).^2+(Linex).^2) - r2 ).^2 /(D/2)^2);

circle3 = (((LineZ-centerz).^2+(Linex).^2 >= (r3 - D/2)^2)...
    &((LineZ-centerz).^2+(Linex).^2 <= (r3 + D/2)^2));
parabol3 = (1 -abs(sqrt((LineZ-centerz).^2+(Linex).^2) - r3 ).^2 /(D/2)^2);

speed = 10e-3;
angular_velocity1 = speed./sqrt((LineZ-centerz).^2+(Linex).^2); 
angular_velocity1 =angular_velocity1 .*circle1.*parabol1;
theta1 = angular_velocity1 .* deltat;

speed = 10e-3;
angular_velocity2 = speed./sqrt((LineZ-centerz).^2+(Linex).^2); 
angular_velocity2 =angular_velocity2 .*circle2.*parabol2;
theta2 = angular_velocity2 .* deltat * (-1);

speed = 10e-3;
angular_velocity3 = speed./sqrt((LineZ-centerz).^2+(Linex).^2); 
...
numberofbubbles1 =  pi * (D/2 *1e3)^2 * len1 * 300/4;
numberofbubbles1 = ceil(numberofbubbles1) ;
xu = rand(numberofbubbles1, 1);
xu = xu*(2*r1 + D) - (r1 +D/2);
d = rand(numberofbubbles1, 1);
d = d * (D) + (r1 - D/2);
sign = rand(numberofbubbles1, 1);
sign(sign >0.5) =1;
sign(sign <=0.5) = -1;
zu = sign.*sqrt(abs(d.^2 - xu.^2)) +centerz;
positions1(:,1)=xu;
positions1(:,2)=zu;
...
numberofbubbles2 =  pi * (D/2 *1e3)^2 * len2 * 300 /4;
numberofbubbles2 = ceil(numberofbubbles2) ;
xr = rand(numberofbubbles2, 1);
xr = xr*(2*r2 + D) - (r2 +D/2);
dr = rand(numberofbubbles2, 1);
dr = dr * (D) + (r2 - D/2);
sign = rand(numberofbubbles2, 1);
sign(sign >0.5) =1;
sign(sign <=0.5) = -1;
zr = sign.*sqrt(abs(dr.^2 - xr.^2)) +centerz;
positions2(:,1)=xr;
positions2(:,2)=zr;
...
numberofbubbles3 =  pi * (D/2 *1e3)^2 * len3 * 300 /4;
numberofbubbles3 = ceil(numberofbubbles3) ;
xm = rand(numberofbubbles3, 1);
xm = xm*(2*r3 + D) - (r3 +D/2);
dm = rand(numberofbubbles3, 1);
dm = dm * (D) + (r3 - D/2);
sign = rand(numberofbubbles3, 1);
sign(sign >0.5) =1;
sign(sign <=0.5) = -1;
zm = sign.*sqrt(abs(dm.^2 - xm.^2)) +centerz;
positions3(:,1)=xm;
positions3(:,2)=zm;
...
data = sum(datatemp,4);
%save('cocentricvesselsoldschool.mat','data','-v7.3');
